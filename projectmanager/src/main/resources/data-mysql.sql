Alter table test.users drop foreign key FK_USER_TASK;
Alter table test.users drop foreign key FK_USER_PROJECT;
Alter table users add constraint FK_USER_PROJECT foreign key (project_id) references project (id) on delete cascade ON UPDATE cascade ;
Alter table users add constraint FK_USER_TASK foreign key (task_id) references task (task_id) on delete cascade ON UPDATE cascade ;
